...
import { isServerRender } from "./env.js";
...
class MyReactComponent extends MyReactComponentInternalInstance {
  ...
  setState = (newValue, callback) => {
    ...
    // if there are more than once call setState function
    this.__nextState__ = Object.assign({}, this.__nextState__, newState);

    if (callback) this.__pendingCallback__.push(callback);
...
function processComponentInstanceLifeCircle(fiber) {
  ...
  // still inject instance state here once the constructor not set it
  ...
  // once there are not have a Provider
...
function processShouldComponentUpdateLifeCircle(fiber) {
  if (
    fiber.instance.shouldComponentUpdate &&
    typeof fiber.instance.shouldComponentUpdate === "function"
  ) {
