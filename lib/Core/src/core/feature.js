...
const nextWorkAsync = (fiber) => {
  const liveFiber = fiber.__newestFiber__.current;

  currentRunningFiber.current = liveFiber;

  let children = [];

  if (liveFiber.__isDynamicNode__) children = nextWorkComponent(liveFiber);
  else if (liveFiber.__isObjectNode__) children = nextWorkObject(liveFiber);
  else if (!liveFiber.__isTextNode__) children = nextWorkCommon(liveFiber);
