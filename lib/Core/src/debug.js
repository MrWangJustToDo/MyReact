 ...
 */
const trackDevLog = (fiber, showDisplayName = true) => {
  const vdom = fiber.__vdom__;
  const source = vdom?.props?.__source;
  const displayName = vdom?.type?.displayName || "";
  const preString = showDisplayName && displayName ? ` - ${displayName} ` : "";
  if (source) {
    const { fileName, lineNumber } = source;
    return `${preString} (${fileName}:${lineNumber})`;
  } else {
    return `${preString}`;
  }
};
...
const getFiberNodeName = (fiber) => {
  ...
  if (fiber.__isLazy__) return `<Lazy />${trackDevLog(fiber)}`;
  if (fiber.__isPortal__) return `<Portal />${trackDevLog(fiber)}`;
  if (fiber.__isSuspense__) return `<Suspense />${trackDevLog(fiber)}`;
