...
class Element {
  ...
  serializeAttrs() {
    ...
    if (attrsKeys.length) {
      return attrsKeys
        .map((key) =>
          this.attrs[key] === true ? key : `${key}=${this.attrs[key]}`
        )
        .reduce((p, c) => `${p} ${c}`, "");
    } else {
      return "";
...
function renderToString(element) {
  isServerSide.current = true;

  const container = new Element("");

  rootContainer.current = container;

  const _rootFiber = createFiberNode({ deepIndex: 0, dom: container }, element);
...
export { updateDom, createDom, render, renderToString, findDOMNode };
