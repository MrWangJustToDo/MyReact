...
function appendToFragment(fiber, parentFiber) {
  const nextFiber = parentFiber.fiberNext;
  if (nextFiber) {
    insertBefore(
      fiber,
      getDom(nextFiber, (fiber) => fiber.fiberFirstChild),
      getDom(fiber.fiberParent, (fiber) => fiber.fiberParent) ||
        rootContainer.current
    );
...
function commitPosition(fiber) {
  ...
  for (let i = children.length - 1; i >= 0; i--) {
    ...
    if (fiber.diffMount) {
      if (fiber.fiberPrevRender) {
        insertBefore(
          fiber,
          getDom(fiber.fiberPrevRender, (fiber) => fiber.fiberFirstChild),
          getDom(fiber.fiberParent, (fiber) => fiber.fiberParent) ||
            rootContainer.current
        );
      } else {
        ...
        if (fiber.fiberNext) {
          insertBefore(
            fiber,
            getDom(fiber.fiberNext, (fiber) => fiber.fiberFirstChild),
            getDom(fiber.fiberParent, (fiber) => fiber.fiberParent) ||
              rootContainer.current
          );
        } else {
