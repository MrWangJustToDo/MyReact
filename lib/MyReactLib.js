...
// 使用单一变量存储，无论是否出现错误，都需要重置当前指针
let currentUpdateFiber = null;
// 记录下最小的 fiberIndex 确定触发到哪一层
let minFiberIndex = Infinity;
...
const debounceCommit = debounce(() => commitStart(updateRootFiber), 0);
...
function transformChildren(fiber, children) {
  ...
  // console.log(fiber.alternate && fiber.alternate.vdom.props.children);
  ...
  while (index < children.length || previousFiber) {
    ...
    if (sameType) {
      ...
      // 不能直接默认使用，可能会存在底层没有更新的情况 使用更新栈进行追踪   todo 
      if (
        newFiber.effect === "NOTHING" &&
        previousFiber.child &&
        fiber.fiberIndex + 1 <= minFiberIndex
      ) {
        // 一点性能优化
        // console.log("保留旧fiber", previousFiber.child);
        newFiber.child = previousFiber.child;
        previousFiber.child.parent = newFiber;
        let temp = previousFiber.child;
        while (temp.slibing) {
          temp = temp.slibing;
          temp.parent = newFiber;
        }
      }
...
function asyncLoop() {
  if (currentUpdateFiber && !needLoop) {
    try {
      const currentUpdateRootFiber = replaceFiberTree(currentUpdateFiber);
      startLoop(currentUpdateRootFiber);
    } catch (e) {
      console.log(e);
    } finally {
      currentUpdateFiber = null;
    }
  }
}
...
const updateFiberArray = [];

function pushFiber(fiber) {
  if (fiber.fiberIndex < minFiberIndex) {
    minFiberIndex = fiber.fiberIndex;
  }
  // 标记推送的fiber
  fiber.needUpdate = true;
  // 放入待更新更新队列  todo
  updateFiberArray.push(fiber);
  if (currentUpdateFiber) {
    if (fiber.fiberIndex < currentUpdateFiber.fiberIndex) {
      currentUpdateFiber = fiber;
      debounceUpdate();
    } else if (fiber.fiberIndex === currentUpdateFiber.fiberIndex) {
      if (fiber.type !== currentUpdateFiber.type) {
        pushFiber(fiber.parent);
      } else {
        debounceUpdate();
      }
    }
...
function commitStart(currentFiber) {
  ...
  minFiberIndex = Infinity;
  updateRootFiber = null;
  needLoop = false;
  if (updateFiberArray.some(fiber => fiber.needUpdate)) {
    console.log('还有需要更新的', updateFiberArray.filter(it => it.needUpdate));
  }
  needDelete = [];
}
