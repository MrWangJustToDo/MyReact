...
function useState(init) {
  ...
  const oldHook =
    currentHookFiber &&
    currentHookFiber.alternate &&
    currentHookFiber.alternate.hookObj &&
    currentHookFiber.alternate.hookObj.hook[currentHookIndex];
  if (oldHook) {
    // 增加当前函数组件的hook索引
    hookIndex++;
    return oldHook;
    ...
    // 是初始化
    if (!currentHookFiber.hookObj) {
      currentHookFiber.hookObj = {
        hookFiber: currentHookFiber,
        hook: [],
      };
    }
    function setValue(value) {
      this.hook[currentHookIndex][0] = value;
      const currentHookItem = this.hookFiber;
      const newHookFiber = {
        create: currentHookItem.create,
        alternate: currentHookItem,
        type: currentHookItem.type,
        props: currentHookItem.props,
        dom: currentHookItem.dom,
        parent: currentHookItem.parent,
        // 也可以不直接将之前的hook拿过来，丢失了之前的状态，可以重新复制一份
        hookObj: currentHookItem.hookObj,
      };
      newHookFiber.hookObj.hookFiber = newHookFiber;
      // 替换原始树
      const previousFiber = currentHookItem.previous;
      if (previousFiber) {
        // 通过slibing进行连接
        if (
          previousFiber.slibing &&
          previousFiber.slibing === currentHookItem
        ) {
          // 进行替换
          previousFiber.slibing = newHookFiber;
        } else if (
          previousFiber.child &&
          previousFiber.child === currentHookItem
        ) {
          // 通过child进行连接
      ...
      // currentHookFiber.child = null;
      currentHookItem.parent = null;
      currentHookItem.previous = null;
      currentHookItem.alternate = null;
      startLoop(newHookFiber);
    }
    currentHookFiber.hookObj.hook[currentHookIndex] = [
      init,
      setValue.bind(currentHookFiber.hookObj),
    ];
    // 因为闭包的原因导致set函数中读取到的currentHookFiber一直会是老的值
    /*
    const item = [
      init,
      (val) => {
        currentHookFiber.hook[currentHookIndex][0] = val;
        const newHookFiber = {
          create: currentHookFiber.create,
          alternate: currentHookFiber,
          type: currentHookFiber.type,
          props: currentHookFiber.props,
          dom: currentHookFiber.dom,
          parent: currentHookFiber.parent,
          // 也可以不直接将之前的hook拿过来，丢失了之前的状态，可以重新复制一份
          hook: currentHookFiber.hook,
        };
        // 替换原始树
        const previousFiber = currentHookFiber.previous;
        if (previousFiber) {
          // 通过slibing进行连接
          if (
            previousFiber.slibing &&
            previousFiber.slibing === currentHookFiber
          ) {
            // 进行替换
            previousFiber.slibing = newHookFiber;
          } else if (
            previousFiber.child &&
            previousFiber.child === currentHookFiber
          ) {
            // 通过child进行连接
            previousFiber.child = newHookFiber;
          } else {
        ...
        // 不能重置这一步，会导致后续的alternate读取不到，因为transfomr逻辑中使用相关逻辑
        // currentHookFiber.child = null;
        currentHookFiber.parent = null;
        currentHookFiber.previous = null;
        currentHookFiber.alternate = null;
        startLoop(newHookFiber);
      },
    ];
    */
    hookIndex++;
    return currentHookFiber.hookObj.hook[currentHookIndex];
  }
}
