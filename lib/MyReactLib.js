...
function asyncLoop() {
  // 如果更新数组存在需要更新的任务
  if (updateRootFiber) {
    const currentUpdateRootFiber = replaceFiberTree(updateRootFiber);
...
function pushFiber(fiber) {
  if (updateRootFiber) {
    if (fiber.fiberIndex < updateRootFiber.fiberIndex) {
      updateRootFiber = fiber;
    } else if (fiber.fiberIndex === updateRootFiber.fiberIndex) {
      updateRootFiber = fiber.parent;
    }
  } else {
    updateRootFiber = fiber;
  }
