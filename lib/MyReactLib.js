  ...
  */
  ...
  if (typeof type === "function") {
    ...
    if (children.length > 1) {
      ...
      if (children.length !== Array.from(new Set(children.map(it => it.props.key))).length) {
...
function asyncLoop() {
  ...
  if (updateFiberArray.length) {
    const currentUpdateRootFiber = replaceFiberTree(updateFiberArray.peek());
    startLoop(currentUpdateRootFiber);
    updateFiberArray.length = 0;
    updateFiberIndex = {};
  }
}
...
function pushFiber(fiber) {
  if (fiber.fiberIndex in updateFiberIndex) {
    ...
    updateFiberArray.pushValue(fiber);
    debounceUpdate();
  }
}
