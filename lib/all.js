...
function getParentReplaceDom(fiber) {
  if (fiber) {
    if (fiber.replaceDom && fiber.effect === "REPLACE") {
...
function transformChildren(fiber, children) {
  ...
  while (index < children.length || index < previousRenderChildren.length) {
    ...
    if (sameType) {
      ...
      if (previousRenderFiber) {
        if (newFiber.dom) {
          newFiber.effect = "REPLACE";
          if (previousRenderFiber.dom) {
            newFiber.replaceDom = previousRenderFiber.dom;
          } else {
            newFiber.replaceDom = getChildDom(previousRenderFiber);
          }
          newFiber.parentDom = getParentDom(previousRenderFiber.parent);
        } else if (previousRenderFiber.dom) {
          newFiber.effect = "REPLACE";
          newFiber.parentDom = getParentDom(previousRenderFiber.parent);
          newFiber.replaceDom = previousRenderFiber.dom;
        }
      } else if (newFiber.dom) {
        const [replaceDom, parentDom] = getParentReplaceDom(fiber);
        if (replaceDom) {
          newFiber.effect = "REPLACE";
          newFiber.replaceDom = replaceDom;
  ...
  if (currentFiber) {
    if (currentFiber.dom) {
      if (currentFiber.effect === "REPLACE") {
        // console.log(parentDom, currentFiber.dom, currentFiber.replaceDom);
    ...
    hookNode = createHookNode(
      {
        hookIndex: index,
        value:
          type === "useState" && typeof init === "function" ? init() : init,
        type,
        deps,
      },
      fiber
