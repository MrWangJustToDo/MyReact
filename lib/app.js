...
function render(element, container) {
  ...
  container.setAttribute("render-by", "MyReact");
...
function insertBeforeSibling(fiber, sibling) {
  ...
  if (siblingChild && siblingChild.dom) {
    if (siblingChild.__isPortal__) {
      ...
      insertBefore(fiber, siblingChild.dom, (0, _tools.getDom)(fiber.fiberParent, f => f.fiberParent) || _env.rootContainer.current);
    }
  } else {
...
function appendToParent(fiber, parent) {
  if (parent) {
    if (parent.__isFragmentNode__) {
      ...
      append(fiber, (0, _tools.getDom)(parent, fiber => fiber.fiberParent) || _env.rootContainer.current);
    }
  } else {
    append(fiber, _env.rootContainer.current);
  }
