    ...
    Object.keys(newProps).filter(_prop.isProperty).filter((0, _prop.isNew)(oldProps, newProps)).forEach(function (key) {
      if (key === "className") {
        ...
        if (newProps[key]) {
          ...
          Promise.resolve().then(function () {
            element.focus(); // reset selection area

            element === null || element === void 0 ? void 0 : element.setSelectionRange(element.value.length, element.value.length);
          });
        }
      } else {
  ...
  _createClass(MyReactFiberNode, [{
    ...
    value: function initialContext() {
      if (this.fiberParent) {
        ...
        var contextMap = Object.assign({}, this.fiberParent.__contextMap__, this.__contextMap__);

        if (this.fiberParent.__isContextProvider__) {
          // get context id;
          var contextObject = this.fiberParent.__vdom__.type.Context;
          var contextId = contextObject.id;

          if (contextMap[contextId]) {
            contextMap[contextId].current = this.fiberParent;
          } else {
            contextMap[contextId] = (0, _share.createRef)(this.fiberParent);
          }
        }
...
var getContextFiber = function getContextFiber(fiber, contextObject) {
  if (contextObject) {
    var _fiber$__contextMap__;

    var id = contextObject.id;
    return (_fiber$__contextMap__ = fiber.__contextMap__[id]) === null || _fiber$__contextMap__ === void 0 ? void 0 : _fiber$__contextMap__.current;
