...
var MyReactComponent = /*#__PURE__*/function (_MyReactInstance) {
  ...
  function MyReactComponent(props, context) {
    ...
    _defineProperty(_assertThisInitialized(_this), "setState", function (newValue) {
...
function processComponentDidUpdateLiftCircle(fiber) {
  if (!fiber.instance.__pendingEffect__ && typeof fiber.instance.componentDidUpdate === "function") {
    fiber.instance.__pendingEffect__ = true;
    (0, _update.pushLayoutEffect)(fiber, function () {
...
},{"./core.js":2,"./fiber.js":5,"./share.js":10,"./tools.js":11,"./update.js":12}],2:[function(require,module,exports){
"use strict";
 ...
 * @param {MyReactFiberNode} fiber
 */
function getIsPortalRender(fiber) {
  if (!fiber) return false;
  if (fiber.__isPortal__) return true;
  if (fiber.dom) return false;
  return getIsPortalRender(fiber.fiberChildHead);
}
/**
 *
 * @param {MyReactFiberNode} fiber
...
function processUpdatePosition(fiber, previousRenderChild) {
  ...
  var isPortalRender = getIsPortalRender(previousRenderChild);
  fiber.diffPrevRender = isPortalRender ? null : previousRenderChild;
...
function getNewFiberWithUpdate(newVDom, parentFiber, previousRenderChild, matchedPreviousRenderChild, isSameType) {
  if (isSameType) {
    if (Array.isArray(newVDom)) {
      ...
      if (newVDom.length < matchedPreviousRenderChild.length) {
        (0, _update.pushUnmount)(matchedPreviousRenderChild.slice(newVDom.length));
      }
    ...
    if (!matchedPreviousRenderChild) return null; // console.log(matchedPreviousRenderChild)

    if (matchedPreviousRenderChild.__isTextNode__ && matchedPreviousRenderChild.__vdom__ === newVDom) {
      // console.log(matchedPreviousRenderChild);
      return (0, _fiber3.updateFiberNode)(matchedPreviousRenderChild, parentFiber, newVDom);
    }

    if (matchedPreviousRenderChild.__isPlainNode__ && (0, _tools.isEqual)(matchedPreviousRenderChild.__vdom__.props, newVDom.props)) {
      var _fiber2 = (0, _fiber3.updateFiberNode)(matchedPreviousRenderChild, parentFiber, newVDom);

      if (matchedPreviousRenderChild !== previousRenderChild) {
        processUpdatePosition(_fiber2, previousRenderChild);
      }
    ...
    if (matchedPreviousRenderChild.__isContextProvider__ && (0, _tools.isNormalEqual)(matchedPreviousRenderChild.__vdom__.props.value, newVDom.props.value)) {
      return (0, _fiber3.updateFiberNode)(previousRenderChild, parentFiber, newVDom);
    }

    var _fiber = (0, _fiber3.createFiberNode)({
    ...
    if (_fiber.dom && _fiber.effect) {
      (0, _update.pushUpdate)(_fiber);
    }

    if (matchedPreviousRenderChild !== previousRenderChild) {
      processUpdatePosition(_fiber, previousRenderChild);
    }
  ...
  var fiber = (0, _fiber3.createFiberNode)({
    fiberParent: parentFiber,
    deepIndex: parentFiber.deepIndex + 1,
    effect: newVDom.__isPortal__ ? "PORTAL" : "PLACEMENT"
  }, newVDom === false || newVDom === null ? "" : newVDom);
  processUpdatePosition(fiber, previousRenderChild);
  return fiber;
}
/**
 *
...
function isSameTypeNode(newVDom, previousRenderChild) {
  ...
  if (newVDom instanceof _vdom.MyReactVDom && previousRenderChildVDom instanceof _vdom.MyReactVDom) {
    ...
    if (newVDom.__isObjectNode__) return previousRenderChildVDom.__isObjectNode__ && newVDom.type.type === previousRenderChildVDom.type.type;
...
function getMatchedRenderChildren(newChildren, previousRenderChildren) {
  ...
  var usedState = Array(previousRenderChildren.length).fill(false);
  var assignPreviousRenderChildren = Array(previousRenderChildren.length).fill(null); // 优先处理有key的

  newChildren.forEach(function (vdom, index) {
    if (vdom instanceof _vdom.MyReactVDom && vdom.key !== undefined) {
      var targetIndex = previousRenderChildren.findIndex(function (fiber, _index) {
        return !usedState[_index] && fiber.key === vdom.key;
      });

      if (targetIndex !== -1) {
        usedState[targetIndex] = true;
        assignPreviousRenderChildren[index] = previousRenderChildren[targetIndex];
      }
    }
  }); // 剩下的填满位置

  for (var i = 0, j = 0; i < assignPreviousRenderChildren.length; i++) {
...
function transformChildrenFiber(parentFiber, _children) {
  ...
  var previousRenderChildren = ((_parentFiber$fiberAlt = parentFiber.fiberAlternate) === null || _parentFiber$fiberAlt === void 0 ? void 0 : _parentFiber$fiberAlt.renderedChildren) || [];
...
function nextWorkCommon(fiber) {
  if (fiber.__vdom__.__dynamicChildren__) {
...
},{"./component.js":1,"./env.js":4,"./fiber.js":5,"./tools.js":11,"./update.js":12,"./vdom.js":13}],3:[function(require,module,exports){
"use strict";
...
function HighLight() {
  ...
  _defineProperty(this, "getFiberWithDom", function (fiber) {
    if (fiber) {
      if (fiber.__isTextNode__) return _this.getFiberWithDom(fiber.fiberParent);
      if (fiber.dom) return fiber;
      if (fiber.__isPortal__) return fiber;
      return _this.getFiberWithDom(fiber.fiberParent);
    }
  });

  _defineProperty(this, "highLight", function (fiber) {
    if (_env.enableHighLight.current) {
      if (!fiber.__pendingHighLight__) {
        fiber.__pendingHighLight__ = true;

        var fiberWithDom = _this.getFiberWithDom(fiber);

        var dom = fiberWithDom.__isPortal__ ? fiberWithDom.__vdom__.props.container : fiberWithDom.dom;

        _this.startHighLight(dom, function () {
  ...
  _defineProperty(this, "startHighLight", function (dom, cb) {
    _this.pendingUpdate.push(dom);

    _this.flashPending(cb);
  });

  _defineProperty(this, "flashPending", function (cb) {
    Promise.resolve().then(function () {
      var allDom = _this.pendingUpdate.slice(0);

      _this.pendingUpdate = [];
      return Promise.all(allDom.map(function (dom) {
        var wrapperDom = _this.getHighLight();

        var left = dom.offsetLeft;
        var top = dom.offsetTop;
        var width = dom.offsetWidth + 4;
        var height = dom.offsetHeight + 4;
...
function updateDom(element, oldProps, newProps, fiber) {
  ...
  if (_env.isMounted.current && _env.enableHighLight.current) {
...
},{"./env.js":4,"./fiber.js":5,"./render.js":9,"./share.js":10,"./tools.js":11,"./vdom.js":13}],4:[function(require,module,exports){
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.rootFiber = exports.rootContainer = exports.pendingUpdateFiberArray = exports.pendingUnmountFiberArray = exports.pendingPositionFiberArray = exports.pendingModifyFiberArray = exports.pendingLayoutEffectArray = exports.pendingEffectArray = exports.nextTransformFiberArray = exports.needLoop = exports.isMounted = exports.enableKeyDiff = exports.enableHighLight = exports.enableAsyncUpdate = exports.empty = exports.currentTransformFiberArray = exports.currentRunningFiber = exports.currentHookDeepIndex = exports.currentFunctionFiber = void 0;
...
},{"./tools.js":11}],5:[function(require,module,exports){
"use strict";
...
var MyReactFiberNode = /*#__PURE__*/function () {
   ...
   * @type MyReactVDom
   */
  function MyReactFiberNode(key, deepIndex,
  /**
   * @type MyReactFiberNode
   */
  fiberParent,
  ...
  instance) {
    ...
    _defineProperty(this, "fiberChildHead", null);

    _defineProperty(this, "fiberChildFoot", null);

    _defineProperty(this, "diffPrevRender", null);
    ...
    _defineProperty(this, "fiberSibling", null);
...
},{"./component.js":1,"./dom.js":3,"./hook.js":6,"./share.js":10,"./update.js":12,"./vdom.js":13}],6:[function(require,module,exports){
"use strict";
...
var MyReactHookNode = /*#__PURE__*/function (_MyReactInstance) {
  ...
  function MyReactHookNode(hookIndex, _value, depArray, hookType,
  /**
   * @type MyReactHookNode
   */
  hookNext,
  ...
  _createClass(MyReactHookNode, [{
    ...
    value: function initialResult() {
      if (this.hookType === "useState") {
        this.result = typeof this.value === "function" ? this.value.call(null) : this.value;
      } else if (this.hookType === "useEffect" || this.hookType === "useLayoutEffect") {
        this.effect = true;
    ...
    key: "update",
    value: function update(newAction, newDepArray, newHookType, newFiber) {
...
function createHookNode(_ref, fiber) {
  ...
  var newHookNode = new MyReactHookNode(hookIndex, value, depArray, hookType);
...
function useState(initialValue) {
  var currentHookNode = getHookNode(_env.currentFunctionFiber.current, _env.currentHookDeepIndex.current++, initialValue, null, "useState");
  return [currentHookNode.result, currentHookNode.setValue];
}

function useEffect(action, depArray) {
  getHookNode(_env.currentFunctionFiber.current, _env.currentHookDeepIndex.current++, action, depArray, "useEffect");
}

function useLayoutEffect(action, depArray) {
  getHookNode(_env.currentFunctionFiber.current, _env.currentHookDeepIndex.current++, action, depArray, "useLayoutEffect");
}

function useCallback(action, depArray) {
  return getHookNode(_env.currentFunctionFiber.current, _env.currentHookDeepIndex.current++, action, depArray, "useCallback").result;
}

function useMemo(action, depArray) {
  return getHookNode(_env.currentFunctionFiber.current, _env.currentHookDeepIndex.current++, action, depArray, "useMemo").result;
}

function useRef(value) {
  return getHookNode(_env.currentFunctionFiber.current, _env.currentHookDeepIndex.current++, (0, _tools.createRef)(value), null, "useRef").result;
}

function useContext(Context) {
  return getHookNode(_env.currentFunctionFiber.current, _env.currentHookDeepIndex.current++, Context, null, "useContext").result;
}
},{"./env.js":4,"./fiber.js":5,"./share.js":10,"./tools.js":11,"./update.js":12}],7:[function(require,module,exports){
"use strict";
...
},{"./env.js":4,"./fiber.js":5,"./tools.js":11,"./update.js":12}],8:[function(require,module,exports){
"use strict";
...
var React = {
  ...
  Children: {
    map: function map(children, action) {
      return (0, _tools.map)(children, function (vDom) {
...
},{"./component.js":1,"./dom.js":3,"./hook.js":6,"./tools.js":11,"./vdom.js":13}],9:[function(require,module,exports){
"use strict";
...
function transformCurrent() {
  while (_env.currentTransformFiberArray.current.length) {
    ...
    if (fiber.mount) (_nextTransformFiberAr = _env.nextTransformFiberArray.current).push.apply(_nextTransformFiberAr, _toConsumableArray((0, _core.nextWork)(fiber)));
  }
}
...
function transformNext() {
  while (_env.nextTransformFiberArray.current.length) {
    ...
    if (fiber.mount) (_currentTransformFibe2 = _env.currentTransformFiberArray.current).push.apply(_currentTransformFibe2, _toConsumableArray((0, _core.nextWork)(fiber)));
  }
}
...
var lastAsyncUpdateTime = new Date().getTime();
...
function loopAll(cb) {
  return function (deadLine) {
    ...
    while (!shouldYield && hasNext) {
      if ((_env.currentTransformFiberArray.length || _env.nextTransformFiberArray.current) && !shouldYield) {
        transformAll();
      }

      shouldYield = deadLine && deadLine.timeRemaining() < 1;
      var needContinueLoop = new Date().getTime() - lastAsyncUpdateTime <= 20;

      if (!shouldYield) {
        var nextStartUpdateFiber = (0, _update.getAsyncPendingModifyNextFiber)();
      ...
      shouldYield = deadLine && deadLine.timeRemaining() < 1;
      hasNext = hasNext && (_env.currentTransformFiberArray.current.length || _env.nextTransformFiberArray.current.length || needContinueLoop);
    }
...
},{"./core.js":2,"./env.js":4,"./fiber.js":5,"./mount.js":7,"./tools.js":11,"./update.js":12}],10:[function(require,module,exports){
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.once = exports.MyReactInstance = void 0;
...
exports.once = once;
},{"./tools.js":11}],11:[function(require,module,exports){
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.getNativeEventName = exports.getFiberNodeName = exports.getDom = exports.getContextFiber = exports.createRef = exports.Provider = exports.Portal = exports.Memo = exports.Fragment = exports.ForwardRef = exports.Context = exports.Consumer = void 0;
exports.isEqual = isEqual;
exports.isNew = exports.isGone = exports.isEvent = void 0;
exports.isNormalEqual = isNormalEqual;
exports.safeCall = exports.map = exports.logHook = exports.logFiber = exports.logCurrentRunningFiber = exports.isStyle = exports.isProperty = void 0;
...
},{"./env.js":4,"./fiber.js":5,"./hook.js":6}],12:[function(require,module,exports){
"use strict";
...
exports.getAsyncPendingModifyNextFiber = getAsyncPendingModifyNextFiber;
...
function getSyncPendingModifyFiberArray() {
  var pendingUpdate = _env.pendingModifyFiberArray.current.slice(0).filter(function (f) {
...
function getAsyncPendingModifyNextFiber() {
  while (_env.pendingModifyFiberArray.current.length) {
    var nextFiber = _env.pendingModifyFiberArray.current.shift();
...
function updateAllAsync() {
  _env.needLoop.current = true;
  (0, _render.renderLoopAsync)(function () {
    runPosition();
    runUpdate();
    runUnmount();
    runLayoutEffect();
...
function appendToFragment(fiber, parentFiber) {
  var nextFiber = parentFiber.fiberSibling;
  fiber.effect = null;

  if (nextFiber) {
    insertBefore(fiber, (0, _tools.getDom)(nextFiber, function (fiber) {
      return fiber.fiberChildHead;
    ...
    if (parentFiber.fiberParent.__isFragmentNode__) {
      ...
      append(fiber, (0, _tools.getDom)(parentFiber.fiberParent, function (fiber) {
        return fiber.fiberParent;
      }) || _env.rootContainer.current);
    }
  } else {
...
function commitPosition(fiber) {
  ...
  for (var i = children.length - 1; i >= 0; i--) {
    ...
    if (_fiber.diffMount) {
      if (_fiber.diffPrevRender) {
        insertBefore(_fiber, (0, _tools.getDom)(_fiber.diffPrevRender, function (fiber) {
          return fiber.fiberChildHead;
        ...
        // new create
        if (_fiber.fiberSibling) {
          insertBefore(_fiber, (0, _tools.getDom)(_fiber.fiberSibling, function (fiber) {
            return fiber.fiberChildHead;
          }), (0, _tools.getDom)(_fiber.fiberParent, function (fiber) {
            return fiber.fiberParent;
          }) || _env.rootContainer.current);
          ...
          if (_fiber.fiberParent.__isFragmentNode__) {
            ...
            append(_fiber, (0, _tools.getDom)(_fiber.fiberParent, function (fiber) {
              return fiber.fiberParent;
            }) || _env.rootContainer.current);
          }
        }
...
},{"./dom.js":3,"./env.js":4,"./fiber.js":5,"./render.js":9,"./tools.js":11}],13:[function(require,module,exports){
"use strict";
...
var MyReactVDom = /*#__PURE__*/function () {
  ...
  _createClass(MyReactVDom, [{
    ...
    value: function _initialProps() {
      if (this.__isClassComponent__ && this.type.prototype.isMyReactMemoComponent && this.type.prototype.isMyReactForwardRefRender) {
...
var checkArrayChildrenKey = (0, _share.once)(function (children) {
...
var checkSingleChildrenKey = (0, _share.once)(function (children) {
...
},{"./component.js":1,"./fiber.js":5,"./share.js":10,"./tools.js":11}]},{},[8]);
