...
function processComponentDidMountLiftCircle(fiber) {
  if (!fiber.instance.__pendingEffect__ && typeof fiber.instance.componentDidMount === "function") {
    ...
    (0, _update.pushLayoutEffect)(fiber, () => {
...
function processUpdatePosition(fiber, previousRenderChild) {
  ...
  const isPortalRender = (0, _tools.getIsPortalRender)(previousRenderChild);
...
function getNewFiberWithUpdate(newVDom, parentFiber, previousRenderChild, matchedPreviousRenderChild, isSameType) {
  if (isSameType) {
    ...
    if (Object.is(matchedPreviousRenderChild.__vdom__, newVDom)) {
      const fiber = (0, _fiber.updateFiberNode)(matchedPreviousRenderChild, parentFiber, newVDom);
    ...
    let fiber = (0, _fiber.createFiberNode)({
...
function getMatchedRenderChildren(newChildren, previousRenderChildren) {
  ...
  const assignPreviousRenderChildren = Array(previousRenderChildren.length).fill(null); // 优先处理有key的

  newChildren.forEach((vdom, index) => {
    if (vdom instanceof _vdom.MyReactVDom && vdom.key !== undefined) {
      const targetIndex = previousRenderChildren.findIndex((fiber, _index) => !usedState[_index] && fiber.key === vdom.key);
...
class HighLight {
  ...
  highLight = fiber => {
    if (_env.enableHighLight.current) {
      if (!fiber.__pendingHighLight__) {
        ...
        const dom = fiberWithDom.__isPortal__ ? fiberWithDom.__vdom__.props.container : fiberWithDom.dom;
  ...
  flashPending = cb => {
    Promise.resolve().then(() => {
      ...
      this.pendingUpdate = [];
      return Promise.all(allDom.map(dom => {
...
function render(element, container) {
  ...
  container.setAttribute("root-node", "MyReact");
  container.__vdom__ = rootElement;
  container.__fiber__ = _rootFiber;
  (0, _render.startRender)(_rootFiber);
}
},{"./env.js":5,"./fiber.js":6,"./render.js":10,"./tools.js":12}],4:[function(require,module,exports){
"use strict";
...
function loopAll(cb) {
  ...
  while (!shouldYield) {
    ...
    if (!shouldYield) {
      const nextStartUpdateFiber = (0, _update.getAsyncPendingModifyNextFiber)();
...
function appendToFragment(fiber, parentFiber) {
  ...
  if (nextFiber) {
    insertBefore(fiber, (0, _tools.getDom)(nextFiber, fiber => fiber.fiberChildHead), (0, _tools.getDom)(fiber.fiberParent, fiber => fiber.fiberParent) || _env.rootContainer.current);
    ...
    if (parentFiber.fiberParent.__isFragmentNode__) {
      ...
      append(fiber, (0, _tools.getDom)(parentFiber.fiberParent, fiber => fiber.fiberParent) || _env.rootContainer.current);
    }
  } else {
  ...
  for (let i = children.length - 1; i >= 0; i--) {
    ...
    if (fiber.__diffMount__) {
      if (fiber.__diffPrevRender__) {
        insertBefore(fiber, (0, _tools.getDom)(fiber.__diffPrevRender__, fiber => fiber.fiberChildHead), (0, _tools.getDom)(fiber.fiberParent, fiber => fiber.fiberParent) || _env.rootContainer.current);
      } else {
        // new create
        if (fiber.fiberSibling) {
          insertBefore(fiber, (0, _tools.getDom)(fiber.fiberSibling, fiber => fiber.fiberChildHead), (0, _tools.getDom)(fiber.fiberParent, fiber => fiber.fiberParent) || _env.rootContainer.current);
        } else {
          // last one
          if (fiber.fiberParent.__isFragmentNode__) {
            appendToFragment(fiber, fiber.fiberParent);
          } else {
            append(fiber, (0, _tools.getDom)(fiber.fiberParent, fiber => fiber.fiberParent) || _env.rootContainer.current);
          }
        }
      }
...
class VDomInternalInstance extends _share.MyReactTypeInternalInstance {
  __INTERNAL_STATE__ = {
    __clonedNode__: null,
    __validKey__: false,
    __validType__: false,
