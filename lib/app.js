...
function isSameTypeNode(newVDom, previousRenderChild) {
  ...
  if (newVDom instanceof _vdom.MyReactVDom && previousRenderChildVDom instanceof _vdom.MyReactVDom) {
    if (newVDom.__isDynamicNode__ || newVDom.__isPlainNode__ || newVDom.__isObjectNode__) return newVDom.type === previousRenderChildVDom.type;
...
function insertBefore(fiber, beforeDom, parentDom) {
  ...
  if (fiber.__isPlainNode__) return parentDom.insertBefore(fiber.dom, beforeDom);
...
function append(fiber, parentDom) {
  ...
  fiber.effect = null;
  if (fiber.__isPortal__) return;
  if (fiber.__isPlainNode__) return parentDom.append(fiber.dom);
...
function getPlainNodeDom(fiber) {
  ...
  if (fiber.__isPlainNode__) return fiber.dom;
...
function getInsertBeforeDomFromSiblingAndParent(fiber) {
  ...
  if (parentSibling) {
    if (parentSibling.__isPlainNode__) {
