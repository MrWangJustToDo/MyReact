...
function processFiberNode(fiber, vdom) {
  ...
  fiber.memoProps = fiber.isTextNode ? vdom : vdom.props;
...
function processFiberState(fiber) {
  if (fiber.firstHook) {
    let hookNode = fiber.firstHook;
    while (hookNode) {
      fiber.memoState.push(hookNode.value);
      hookNode = hookNode.nextHook;
    }
  }
}
...
function workLoop() {
  return (deadline) => {
    ...
    if (!currentTransformFiberArray.length) {
      needLoop = false;
      // const canLoopTime = lastUpdateTime && Date.now() - lastUpdateTime <= 60;
  ...
  if (fiber.needUpdate) {
    if (!fiber.isTextNode) {
      ...
      if (fiber.__vdom__.create) {
        // memo  not stable
        // if (fiber.vdom.create.memo && fiber.alternate) {
