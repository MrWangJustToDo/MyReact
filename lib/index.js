...
class MyReactVDom {
  constructor(name, tagName, create, props, children, rawType) {
    ...
    this.key = key;
    this.ref = ref;
    // 在这个地方分离了ref和key  所以函数组件调用中props不会包含ref
    this.props = resProps;
  ...
  _initialProps() {
    ...
    // memo + forwardRef 自动穿透
    if (
      this.ref &&
      this.create &&
      this.create.prototype.isMyReactMemoComponent &&
      this.create.prototype.isMyReactForwardRefRender
    ) {
...
function processVDomType(vdom) {
  ...
  } else if (rawType === MReact.Portal) {
    vdom.__isPortalNode__ = true;
  } else if (rawType === MReact.ForwardRef) {
    vdom.__isForwardRef__ = true;
...
function processValidChildren(children, type) {
  ...
  if (_children.some((c) => typeof c === "object" && !isValidElement(c))) {
  ...
  if (typeof type === "object" && type?.type === MReact.ForwardRef) {
    if (typeof type.render !== "function") {
      throw new Error("forwardRef() only accept function child");
    }
  }
  return children;
}
...
function _createElement(type, props, children, rawType) {
  ...
  if (type === MReact.Portal)
    return createVDom("Portal", null, null, props, children, rawType);
  if (type === MReact.ForwardRef)
    return createVDom("ForwardRef", null, null, props, children, rawType);
...
function processFiberType(fiber) {
  ...
  if (vdom instanceof MyReactVDom) {
    ...
    fiber.__isPortalNode__ = vdom.__isPortalNode__;

    fiber.__isDynamicNode__ = vdom.__isDynamicNode__;

    fiber.__isObjectNode__ = vdom.__isObjectNode__;

    fiber.__isFragmentNode__ = vdom.__isFragmentNode__;

    fiber.__isForwardRef__ = vdom.__isForwardRef__;
...
function processFiberDom(fiber) {
  ...
  if (!fiber.dom && fiber.__isPortalNode__) {
    fiber.dom = fiber.__vdom__.props.portalContainer;
  }
}
...
function createPortal(element, container) {
  const portalVDom = createElement(
    MReact.Portal,
...
function nextWorkForwardRef(fiber) {
  const { render, ...lastProps } = fiber.__vdom__.props;
  try {
    hookDepIndex = 0;

    currentFunctionFiber = fiber;

    const children = render(lastProps, fiber.__vdom__.ref);
...
function updateInstanceStateAndProps(fiber, newProps, newState, newContext) {
  ...
  if (instance) {
    ...
    if (newContext) {
      instance.__prevContext__ = instance.context;
      instance.context = newContext;
    }
    instance.__nextProps__ = null;
    instance.__nextState__ = null;
    instance.__nextContext__ = null;
  }
...
function processStateFromPropsUpdate(fiber) {
  ...
  // setState触发的更新
  return Object.assign({}, instance.state, _newState, instance.__nextState__);
}

function processShouldComponentUpdate(fiber, newProps, newState, newContext) {
  const instance = fiber.instance;
...
function processUpdateComponentContext(fiber) {
  ...
  if (fiber.instance.__context__ && !fiber.instance.__context__.mount) {
    ...
    return contextFiber.memoProps.value;
  }
...
function processComponentUpdate(fiber) {
  fiber.instance.__fiber__ = fiber;
  const nextState = processStateFromPropsUpdate(fiber);
  const nextContext = processUpdateComponentContext(fiber);
...
function processInitialComponentContext(fiber) {
  const component = fiber.__vdom__.create;
  return getContextFiber(fiber, component.contextType);
}

function processInitialComponentInstance(fiber) {
  const contextFiber = processInitialComponentContext(fiber);
...
function nextWorkContextProvider(fiber) {
  // context更新  所有的依赖节点更新
  const listenersFiber = fiber.listeners.map((it) => it.__fiber__);
  // update only alive fiber
  Promise.resolve().then(() =>
    listenersFiber.filter((f) => f.mount).forEach((f) => f.update())
  );
...
function nextWorkObject(fiber) {
  ...
  if (fiber.initial || fiber.__needUpdate__) {
    ...
    } else if (fiber.__isContextConsumer__) {
      return nextWorkContextConsumer(fiber);
    } else if (fiber.__isForwardRef__) {
...
function isSameTypeNode(vdom, previousVDom) {
  if (previousVDom !== null) {
    ...
    if (vdom instanceof MyReactVDom && previousVDom instanceof MyReactVDom) {
      ...
      if (vdom.__isPortalNode__) return previousVDom.__isPortalNode__;
      if (vdom.__isFragmentNode__) return previousVDom.__isFragmentNode__;
      if (vdom.__isForwardRef__) return previousVDom.__isForwardRef__;
      if (vdom.__isObjectNode__)
        return (
          previousVDom.__isObjectNode__ && vdom.rawType === previousVDom.rawType
        );
...
function getNewFiber(
  ...
  if (isSameType) {
    ...
    // Context Provider
    if (
      previousRenderFiber.__isObjectNode__ &&
    ...
    if (
      previousRenderFiber.__isObjectNode__ &&
      previousRenderFiber.__isForwardRef__ &&
      isNormalEqual(vdom.props, previousRenderFiber.__vdom__.props)
    ) {
      return updateFiberNode(
        previousRenderFiber,
        parentFiber,
    ...
    if (
      previousRenderFiber.__isPortalNode__ &&
      isNormalEqual(vdom.props, previousRenderFiber.__vdom__.props)
    ) {
      return updateFiberNode(
        previousRenderFiber,
    ...
    return createFiberNode(
      {
        ...
        effect: previousRenderFiber.__isPortalNode__
          ? null
          : previousRenderFiber.__isTextNode__ ||
  ...
  return createFiberNode(
    {
      ...
      // 对于portal节点  不需要append
      effect: vdom.__isPortalNode__ ? null : "PLACEMENT",
      last,
    },
    vdom
  );
...
const asyncUpdate = () => Promise.resolve().then(syncLoop);
...
function getNodeNameFromFiber(fiber) {
  ...
  if (fiber.__isPortalNode__) return `<${fiber.__vdom__.name}>`;
  if (fiber.__isForwardRef__) return `<${fiber.__vdom__.name}>`;
...
function memo(MemoRender) {
  class MemoComponent extends MyReactPureComponent {
    ...
    get isMyReactForwardRefRender() {
      return (
        typeof MemoRender === "object" && MemoRender.type === MReact.ForwardRefF
      );
    }
...
function isValidElement(element) {
  return (
    ...
    (element instanceof MyReactVDom || element === null)
  );
}
...
var MReact = {
  ...
  ForwardRef: Symbol.for("ForwardRef"),
