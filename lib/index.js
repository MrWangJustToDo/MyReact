...
class MyReactFiberNode {
  ...
  addListener(hookNode) {
    // 需要remove逻辑
    // this.listeners = this.listeners.filter((h) => h.__fiber__.mount);
    if (this.listeners.every((h) => h !== hookNode)) {
      this.listeners.push(hookNode);
    }
  }
...
function processMemoUpdate(fiber) {
  ...
  fiber.alternate && (fiber.alternate.__needUpdate__ = false);
...
// 保留下之前的监听者
function processFiberListener(fiber, previousRenderFiber) {
  if (fiber && previousRenderFiber && fiber !== previousRenderFiber) {
    fiber.listeners = previousRenderFiber.listeners;
  }
}

function processFiberHighLightCover(fiber, previousRenderFiber) {
  if (previousRenderFiber && fiber !== previousRenderFiber) {
...
function getNewFiber(
  ...
  if (isSameType) {
    ...
    // 如果之前是特殊fiber，也可以复用
    if (
      previousRenderFiber.__isEmptyNode__ ||
      previousRenderFiber.__isFragmentNode__
    ) {
      return updateFiberNode(
        previousRenderFiber,
        parentFiber,
    ...
    // 如果是context fiber，props没改变时也可以复用
    if (
      previousRenderFiber.__isContextNode__ &&
      isEqual(previousRenderFiber.__vdom__.rawProps, vdom.rawProps, false)
    ) {
      return updateFiberNode(
        previousRenderFiber,
        parentFiber,
...
function transformChildren(parentFiber, children) {
  ...
  while (index < children.length || index < previousRenderChildren.length) {
    ...
    isMounted &&
      newFiber &&
      isSameType &&
      processFiberListener(newFiber, previousRenderFiber);
  ...
  if (fiber) {
    ...
    fiber.hookList.forEach((hook) => {
      ...
      if (hook.hookType === "useContext") {
        ...
        hook.context.listeners = hook.context.listeners.filter(
          (h) => h !== hook
        );
      }
    });
