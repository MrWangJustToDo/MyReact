...
function processPreviousRenderFiberState(fiber, previousRenderFiber) {
...
function getNewFiber(
  ...
  if (isSameType) {
    ...
    if (
      !previousRenderFiber.__isTextNode__ &&
      // dynamic 的children存在动态生成的可能，因此必须运行函数
...
function transformChildren(parentFiber, children) {
  ...
  while (index < children.length || index < previousRenderChildren.length) {
    ...
    isMounted &&
      newFiber &&
      processPreviousRenderFiberState(newFiber, previousRenderFiber);
