...
class MyReactComponent {
  ...
  setState = (newValue) => {
    ...
    this.__fiber__.memoState = [this.state, newState];
    this.state = newState;
    if (!Object.is(this.__fiber__.memoState[0], this.__fiber__.memoState[1])) {
      this.forceUpdate();
    }
  };
...
function processComponentInstance(fiber) {
  if (!fiber.instance) {
    ...
    fiber.instance._updateProps(fiber.__vdom__.props);
  }
...
function nextWorkClassComponent(fiber) {
  const instance = processComponentInstance(fiber);
