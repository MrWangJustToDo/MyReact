...
function debounce(action, time) {
  let id;
  return (...args) => {
    if (lastTimeStep && Date.now() - lastTimeStep < 30) {
      clearTimeout(id);
    }
    id = setTimeout(() => {
...
function shouldNotUpdate(fiber, alternate) {
  return isEqual(fiber.memoProps, alternate.memoProps);
}
...
function updateDom(element, oldProps, newProps, fiber, isUpdate = true) {
...
// =========== // instance
class MyReactVDom {
  constructor(name, tagName, create, props, children, rowType) {
...
class MyReactFiberNode {
  constructor(
    ...
    this.__highLightCover__ = __highLightCover__;
    this.processState();
    this.processUpdate();
    this.processType();
  }
...
// ============ //  core
function createVDom(name, tagName, create, props, children, rowType) {
  const vdom = new MyReactVDom(name, tagName, create, props, children, rowType);
...
function cloneElement(element, props, ...children) {
  if (element instanceof MyReactVDom) {
    let clonedElement = createElement(element.rowType, props, ...children);
...
function createDom(fiber) {
  ...
  updateDom(
    dom,
    empty,
    fiber.__isTextNode__ ? empty : fiber.__vdom__.rowProps,
    fiber,
    false
  );
...
function processFiberNode(fiber, vdom) {
  ...
  fiber.memoProps = !fiber.__isTextNode__ ? vdom.rowProps : [];
  ...
  if (fiber.initial || fiber.__needUpdate__) {
    ...
    if (fiber.__isDynamicNode__) {
      ...
      try {
        ...
        // 不能使用rowProps 进行render  会丢失node类型
...
function isSameTypeNode(newVDom, previousRenderFiber) {
  if (typeof newVDom !== "object" && previousRenderFiber)
    return typeof previousRenderFiber.__vdom__ !== "object";

  if (newVDom instanceof MyReactVDom && previousRenderFiber) {
    if (newVDom.__isEmptyNode__)
      return previousRenderFiber.__vdom__.__isEmptyNode__;
    if (newVDom.__isPlainNode__)
      return newVDom.tagName === previousRenderFiber.__vdom__.tagName;
    if (newVDom.__isDynamicNode__)
      return newVDom.create === previousRenderFiber.__vdom__.create;
  }
  if (previousRenderFiber && previousRenderFiber.__isEmptyNode__) return true;
  if (newVDom && previousRenderFiber)
    return isEqual(newVDom, previousRenderFiber.__vdom__);
...
function processPreviousRenderFiber(previousRenderFiber, unmount) {
  if (previousRenderFiber) {
    ...
    // previousRenderFiber.__needUpdate__ = false;
    if (unmount) {
...
function transformChildren(parentFiber, children) {
  ...
  while (index < children.length || index < previousRenderChildren.length) {
    ...
    const sameType = isSameTypeNode(newVDom, previousRenderFiber);
        ...
        newFiber = createFiberNode(
          {
            ...
            effect: previousRenderFiber.__isTextNode__
              ...
              ? isEqual(
                  newVDom.rowProps,
                  previousRenderFiber.__vdom__.rowProps,
                  false
                )
                ? "NOTHING"
                : "UPDATE"
              : "NOTHING",
            __highLightCover__: previousRenderFiber.__highLightCover__,
          },
          newVDom
        );
      }
    ...
    processFiberReplaceEffect(newFiber, previousRenderFiber, sameType);
    ...
    if (currentFiber.dom) {
      if (currentFiber.effect === "REPLACE") {
        ...
        updateDom(
          ...
          currentFiber.__isTextNode__ ? empty : currentFiber.__vdom__.rowProps,
          currentFiber
        );
      }
  ...
  if (!needLoop) {
    ...
    while (_modifiedFiberArray.length) {
      ...
      fiber.__needUpdate__
        ? startLoop(fiber)
        : console.log("updated", logFiber(fiber));
    }
    commitStart();
  } else {
    clearTimeout(id);
