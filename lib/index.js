...
function highLightDom(fiber, dom) {
  if (fiber.isPlainNode) {
    dom.style.boxShadow = "1px 1px 1px red";
    dom.style.transform = "boxShadow 0.3s";
    setTimeout(() => {
...
function createDom(fiber) {
  ...
  updateDom(dom, empty, fiber.isTextNode ? empty : fiber.__vdom__.props, fiber);
...
function workLoop(firstRender) {
  return (deadline) => {
    ...
    if (!currentTransformFiberArray.length) {
      ...
      // const canLoopTime = lastUpdateTime && Date.now() - lastUpdateTime <= 16;
...
function getHookNode(fiber, hookIndex, hookType, value, depArray) {
  ...
  if (fiber) {
    if (fiber.__hookList__.length > hookIndex) {
      hookNode = fiber.__hookList__[hookIndex];
      if (!hookNode) {
...
function clearFiberHook(fiber) {
  if (fiber) {
    if (fiber.child) {
      fiber.__children__.forEach(clearFiberHook);
    }
    if (fiber.firstHook) {
      let hook = fiber.firstHook;
      while (hook) {
...
function clearFiberDom(fiber) {
  if (fiber.dom) {
    ...
    const childrenFiber = fiber.__children__.slice(0);
