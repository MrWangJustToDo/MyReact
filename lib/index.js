...
function processVDomType(vdom) {
  if (vdom.rawType === MReact.Fragment) {
    vdom.__isFragmentNode__ = true;
  } else if (vdom.rawType === MReact.Context) {
...
function createElement(type, props, ...children) {
  ...
  if (typeof type === "object" && type !== null)
    return cloneElement(type.children, { ...props, __by__: type }, ...children);

  if (type === MReact.Fragment)
    return createVDom("Fragment", null, null, props, children, type);

  if (type === MReact.Context)
    return createVDom("Context", null, null, props, children, type);
...
function getNewFiber(
  ...
  if (isSameType) {
    ...
    if (previousRenderFiber.effect) {
      console.log("update fiber node", fiber.effect, fiber);
...
function getContextFiber(fiber, ContextObject) {
  if (fiber) {
    if (
      ...
      fiber.__vdom__.props.__by__ === ContextObject
    ) {
      return fiber;
    } else {
...
function processHookNodeValue(hookNode, fiber) {
  if (hookNode.hookType === "useState") {
    ...
    const Context = getContextFiber(fiber, hookNode.value);
    hookNode.context = Context;
    hookNode.result = Context ? Context.memoProps.value : undefined;
  } else {
    hookNode.result = hookNode.value;
  }
}
...
function updateHookNode(hookNode, newAction, newDepArray, newFiber) {
  if (hookNode.hookType === "useEffect") {
    ...
    if (!isNotChange || !hookNode.context.mount) {
      // console.log('new run', isNotChange, hookNode.context.mount)
      hookNode.value = newAction;
      const Context = getContextFiber(newFiber, hookNode.value);
      hookNode.result = Context ? Context.memoProps.value : undefined;
    }
  }
}
  ...
  if (fiber.__isEmptyNode__)
    return `<${"empty"} - (${
      typeof fiber.__vdom__ === "object"
        ? JSON.stringify(fiber.__vdom__.props)
...
function createContext(value) {
  ...
  return {
    children: MReact.createElement(MReact.Context, {
      value: defaultValue,
    }),
  };
