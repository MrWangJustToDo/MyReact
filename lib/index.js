...
function processFiberNode(fiber, vdom) {
  ...
  fiber.memoProps = fiber.__isTextNode__ ? vdom : vdom.rowProps;
...
function nextWork(fiber) {
  ...
  if (fiber.needUpdate) {
    ...
    } else if (!fiber.__isTextNode__ && fiber.__vdom__.children) {
      childrenFiberArray = transformChildren(fiber, fiber.__vdom__.children);
    } else {
      // console.log("last", fiber);
    }
  } else {
...
function processPreviousRenderFiber(previousRenderFiber, unmount) {
  if (previousRenderFiber) {
    // 是一个被跳过的更新
    if (previousRenderFiber.__isUpdate__) {
      previousRenderFiber.__isUpdate__ = false;
    } else {
...
function log(fiber) {
  if (fiber.__root__) {
    ...
    let re =
      "fond in --> " +
      `<${
        fiber.__vdom__.name ? fiber.__vdom__.name : fiber.__vdom__.toString()
      } />`;
    while (fiber.parent) {
      fiber = fiber.parent;
      if (fiber.__root__) {
