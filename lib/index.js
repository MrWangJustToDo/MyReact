...
function createVDom(name, tagName, create, props, children) {
  const vdom = new MyReactVDom(name, tagName, create, props, children);
...
function processFiberNode(fiber, vdom) {
  ...
  fiber.memoProps = fiber.__isDynamicNode__ ? vdom.rowProps : [];
}
...
function pushFiber(fiber) {
  if (!fiber.__needUpdate__) {
    if (!fiber.mount) {
      console.log("update unmount fiber", fiber);
      return;
    }
...
function getNodeNameFromFiber(fiber) {
  ...
  if (fiber.__isDynamicNode__ || fiber.__isPlainNode__)
    return `<${fiber.__vdom__.name} />`;
