...
function processCheckChildrenKey(children) {
  // check children
  if (
    children.some(
...
function processFiberReplaceEffect(fiber, previousRenderFiber) {
  ...
  if (previousRenderFiber) {
    ...
    const replaceDom = getDom(previousRenderFiber, (fiber) => fiber.child);
    if (replaceDom) {
      fiber.effect = "REPLACE";
      fiber.replaceDom = replaceDom;
...
// function getPreviousVDom(vdom, previousRenderChildren) {
...
function transformChildren(parentFiber, children) {
  ...
  while (index < children.length || index < previousRenderChildren.length) {
    ...
    isMounted && !isSameType && console.log(newVDom, previousVDom);
