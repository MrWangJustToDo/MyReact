...
function nextWork(fiber) {
  if (!fiber.child) {
    if (typeof fiber.vdom === "object" && fiber.vdom.create) {
      hookIndex = 0;
...
function transformChildren(fiber, children) {
  ...
  while (index < children.length || previousFiber) {
    ...
    if (sameType) {
      ...
      const textNode = typeof newVdom !== "object";
      newFiber = {
        vdom: newVdom,
        key: !textNode ? newVdom.key : null,
        parent: fiber,
        alternate: previousFiber,
        effect: "PLACEMENT",
        dom: null,
      };
      !textNode && (newVdom.fiber = newFiber);
...
function createDom(fiber) {
  ...
  // 文本节点
  if (typeof fiber.vdom !== "object") {
    dom = document.createTextNode(fiber.vdom);
  } else {
    dom = document.createElement(fiber.vdom.type);
