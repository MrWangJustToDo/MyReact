...
function highLightDom(fiber) {
  if (fiber.isPlainNode) {
    const dom = fiber.dom;
    if (!fiber.highLightCover) {
      const cover = document.createElement("div");
      highLightDomContainer.append(cover);
      fiber.highLightCover = cover;
    }
    ...
    fiber.highLightCover.style.cssText = `
      position: absolute;
      width: ${width}px;
      height: ${height}px;
      ...
      `;
    setTimeout(() => {
      fiber.highLightCover.style.boxShadow = "none";
    }, 100);
  } else {
...
class MyReactFiberNode {
  ...
  processFiber() {
    this.lastUpdateTimeStep = this.alternate
      ? this.alternate.updateTimeStep
      : null;
    this.updateTimeStep = Date.now();
...
function processFiberDom(fiber, vdom) {
  ...
  fiber.isPlainNode = vdom.__isPlainNode__;

  fiber.isDynamicNode = vdom.__isDynamicNode__;
...
function isSameTypeNode(newVDom, previousRenderFiber, isTextNode) {
  ...
  } else if (newVDom.__isPlainNode__) {
    return (
      previousRenderFiber &&
      previousRenderFiber.isPlainNode &&
      newVDom.tagName === previousRenderFiber.__vdom__.tagName
    );
  } else if (newVDom.__isDynamicNode__) {
    return (
      previousRenderFiber &&
      newVDom.create === previousRenderFiber.__vdom__.create
    );
  }
  return false;
}
        ...
        newFiber = createFiberNode(
          {
            ...
            effect: isTextNode
              ...
              ? "NOTHING"
              : "UPDATE",
            highLightCover: previousRenderFiber.highLightCover,
          },
          newVDom
        );
      }
  ...
  if (currentFiber && currentFiber.mount) {
    if (currentFiber.dom) {
      if (currentFiber.effect === "REPLACE") {
        ...
        updateDom(
          currentFiber.dom,
          currentFiber.isTextNode
            ? empty
            : currentFiber.alternate.__vdom__.rowProps,
          currentFiber.isTextNode ? empty : currentFiber.__vdom__.rowProps,
          currentFiber
        );
      }
...
function processUpdateFiber(fiber) {
  let _fiber = fiber;
  // 追踪更新链路
  while (fiber && !fiber.needUpdate && !fiber.__recursive__) {
...
function clearFiberHighLight(fiber) {
  if (fiber) {
    if (fiber.highLightCover) fiber.highLightCover.remove();
