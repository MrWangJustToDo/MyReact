...
class MyReactHookNode {
  ...
  update() {
    if (!this.__fiber__) {
      throw new Error("只能在component中使用hook");
    } else {
      pushFiber(this.__fiber__);
    }
  }
}
...
function createElement(type, props, ...children) {
  ...
  if (typeof type === "function") {
    ...
    return createVDom(type, type, null, props, children, type);
  }
}
...
function processFiberType(fiber) {
  if (fiber.__isSkipUpdate__) return;
  ...
  if (vdom instanceof MyReactVDom) {
    ...
    // 当使用<>{props.children}</> 时会发生  好像不会发生
...
function processFiberRef(fiber) {
  const vdom = fiber.__isTextNode__ ? {} : fiber.__vdom__;
  const { ref } = vdom;
...
function processFiberNode(fiber, vdom) {
  ...
  if (!fiber.__isTextNode__) vdom.__fiber__ = fiber;
}
...
function mountLoop(currentFiber, parentDom) {
  if (currentFiber && currentFiber.mount) {
    ...
    mountLoop(currentFiber.child, currentFiber.dom || parentDom);
    mountLoop(currentFiber.slibing, parentDom);
...
function startUpdate(fiber) {
  if (fiber.mount) {
    ...
    asyncUpdate ? updateLoopAsync() : updateLoopSync();
  }
}
...
function updateLoopSync() {
  ...
  while (_modifiedFiberArray.length) {
    const fiber = _modifiedFiberArray.shift();
    if (fiber.mount && fiber.__needUpdate__) {
      startUpdate(fiber);
    }
  }
  needLoop = false;
...
function workLoop() {
  return (deadline) => {
    ...
    if (!currentTransformFiberArray.length && !nextTransformFiberArray.length) {
      let nextUpdateFiber = null;
      while (_modifiedFiberArray.length && !nextUpdateFiber) {
        const fiber = _modifiedFiberArray.shift();
        if (fiber.mount && fiber.__needUpdate__) {
          nextUpdateFiber = fiber;
        }
      }
      if (nextUpdateFiber) {
        startUpdate(nextUpdateFiber);
      } else {
        needLoop = false;
...
function nextWork(fiber) {
  // 这是一个新创建出来的fiber或者是一个主动触发更新的fiber
...
function isSameTypeNode(vdom, previousVDom) {
  if (
    (vdom !== undefined && previousVDom === undefined) ||
    (vdom === undefined && previousVDom !== undefined)
  )
    return false;
  if (typeof vdom !== "object" && previousVDom !== undefined)
    return typeof previousVDom !== "object";
  if (vdom.__isEmptyNode__ && previousVDom) return previousVDom.__isEmptyNode__;
  if (vdom.__isDynamicNode__ && previousVDom)
    return vdom.create === previousVDom.create;
  if (vdom.__isPlainNode__ && previousVDom)
    return vdom.tagName === previousVDom.tagName;

  return false;
}

function processFiberReplaceEffect(fiber, previousRenderFiber, sameType) {
  if (fiber) {
    if (!sameType) {
...
function processPreviousRenderFiberState(previousRenderFiber) {
  if (previousRenderFiber) {
    // 是一个被跳过的更新
    if (previousRenderFiber.__isSkipUpdate__) {
      previousRenderFiber.__isSkipUpdate__ = false;
    } else {
...
function processFiberHighLightCover(fiber, previousRenderFiber, isSameType) {
  if (isSameType && previousRenderFiber && fiber !== previousRenderFiber) {
    ...
    return createFiberNode(
      {
        ...
        effect: previousRenderFiber.__isTextNode__
          ? vdom === previousRenderFiber.__vdom__
            ? "NOTHING"
            : "UPDATE"
...
function transformChildren(parentFiber, children) {
  ...
  while (index < children.length || index < previousRenderChildren.length) {
    ...
    isMounted &&
      processFiberReplaceEffect(newFiber, previousRenderFiber, isSameType);

    isMounted && processParentFiberReplaceEffect(parentFiber, newFiber);

    isMounted && processPreviousRenderFiberState(previousRenderFiber);

    isMounted &&
      processFiberHighLightCover(newFiber, previousRenderFiber, isSameType);

    // 对于挂载的时候 此时的fiber上还没有hook
    // pushEffect(newFiber);

    previousRenderFiber && !isSameType && pushUnmount(previousRenderFiber);
...
function processValidHookNode(hookNode) {
  if (
    ...
    if (typeof hookNode.value !== "function") {
      degWithFiber("processCheckHookNode", `${hookNode.hookType} 初始化错误`);
...
function processHookNodeValue(hookNode) {
...
function asyncLoop() {
  if (!needLoop) {
    ...
    modifiedFiberArray = [];
    if (_modifiedFiberArray.length) {
      startUpdate(_modifiedFiberArray.shift());
    }
  }
}
...
function pushEffect(fiber) {
  fiber.hookList.forEach((hook) => {
    if (hook.hookType === "useEffect" && hook.effect) {
      ...
      needEffectHookNodeArray.unshift(hook);
    }
  });
}
...
function getNodeNameFromFiber(fiber) {
  ...
  if (fiber.__isEmptyNode__)
    return `<empty - (${JSON.stringify(fiber.__vdom__)}) />`;
