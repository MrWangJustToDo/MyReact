...
function debugWithCurrentFiber(...args) {
  if (deg) {
    console.log(
      "[debug]",
      ...args.map(
        (it) => (it instanceof MyReactFiberNode ? log(it) : it),
...
/*
function updateFiberNodeReplaceCreate(fiber, parent, previous, newVDom) {
  fiber.parent = parent;
  fiber.previous = previous;
  fiber.__children__ = [];
  fiber.__hookList__ = [];
  fiber.key = newVDom.key;
}
*/
    ...
    if (!fiber.isTextNode) {
      ...
      if (fiber.__vdom__.create) {
        ...
        if (fiber.__vdom__.children.length) {
          debugWithCurrentFiber("nextWork", "函数组件存在children", fiber.__vdom__.children);
        }
        try {
