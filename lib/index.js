...
class MyReactPureComponent extends MyReactComponent {
  ...
  render() {
    const { Render, ...resProps } = this.props;
    return createElement(Render, resProps);
  }
}
...
// TODO  提前将previousChildren顺序排好  匹配当前的vdom Children
function processPreviousRenderChildren(vdomChildren, previousRenderChildren) {
  ...
  vdomChildren.forEach((vdom, index) => {
    if (vdom instanceof MyReactVDom && vdom.key !== undefined) {
      const targetIndex = previousRenderChildren.findIndex((fiber, _index) => {
        !usedState[_index] && fiber.key === vdom.key;
      });
      console.log(vdom, targetIndex, previousRenderChildren);
      if (targetIndex !== -1) {
...
function positionUpdate(fiber) {
  ...
  for (let i = children.length - 1; i >= 0; i--) {
    ...
    if (!fiber.alternate && fiber.last) {
      insertBefore(
        fiber,
        getDom(fiber.last, (fiber) => fiber.child),
...
function pushEffect(node) {
  if (!node.__pendingEffect__) {
    ...
    node.hookType === "useLayoutEffect"
      ? needLayoutEffectNodeArray.unshift(node)
      : needEffectNodeArray.unshift(node);
  }
}
