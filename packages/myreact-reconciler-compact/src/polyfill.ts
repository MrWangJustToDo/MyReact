...
async function loadRemoteScript(
  url: string, 
  options: LoadRemoteScriptOptions = {}
): Promise<void> {
...
async function executeScript(
  code: string, 
  url: string, 
  options: { context?: Record<string, any>; useEval?: boolean }
): Promise<void> {
...
async function executeInBun(code: string, url: string, context: Record<string, any>): Promise<void> {
  try {
    ...
    if (Bun.compile) {
      try {
        // @ts-ignore
        const compiled = Bun.compile(code);
        await compiled.run();
        return;
      } catch (compileError) {
        console.warn('Bun.compile failed:', compileError.message);
      }
    }
