...
export const setLogScope = () => {
  if (__DEV__) {
    ...
    console.warn = (...args) => originalWarn(...args, renderPlatform.getFiberTree(currentRunningFiber.current));

    console.error = (...args) => originalError(...args, renderPlatform.getFiberTree(currentRunningFiber.current));
  }
};
