...
import { nonAlphanumericKeys, parseKeypress } from "../api";

import { useStdin } from "./use-stdin";

const { currentRenderPlatform } = __my_react_internal__;

const isUpperCase = (input: string) => input.toUpperCase() === input && input.toLowerCase() !== input;
  ...
  useEffect(() => {
    ...
    const handleData = (data: string) => {
      ...
      if (!(input === "c" && key.ctrl) || !internal_exitOnCtrlC) {
        currentRenderPlatform.current.yieldTask(() => inputHandler(input, key));
      }
    };
