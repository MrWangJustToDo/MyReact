...
import type { EventManager, Events, DomEvent } from "../core/events";
import type { RootState, RootStore } from "../core/store";
...
export function createPointerEvents(store: RootStore): EventManager<HTMLElement> {
  ...
  return {
    ...
    connected: undefined,
    handlers: Object.keys(DOM_EVENTS).reduce((acc, key) => ({ ...acc, [key]: handlePointer(key) }), {}) as unknown as Events,
    update: () => {
