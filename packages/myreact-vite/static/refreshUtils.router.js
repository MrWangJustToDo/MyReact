...
const enqueueUpdate = debounce(async () => {
  ...
  if (routeUpdates.size > 0) {
    ...
    for (let route of routeUpdates.values()) {
      ...
      if (!imported) {
        throw Error(`[react-router:hmr] No module update found for route ${route.id}`);
      }
      let routeModule = {
        ...imported,
        ...
        HydrateFallback: imported.HydrateFallback
          ? (window.__reactRouterRouteModules[route.id]?.HydrateFallback ?? imported.HydrateFallback)
    ...
    let needsRevalidation = new Set(
      Array.from(routeUpdates.values())
        .filter((route) => route.hasLoader || route.hasClientLoader || route.hasClientMiddleware)
        .map((route) => route.id)
    );
