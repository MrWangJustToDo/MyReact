...
const generateModuleLoader = (loadOption) => {
  const moduleLoader = async ({ entry, fullPath }, option) => {
    ...
    const { getModuleContent, getModuleDeps, generateModuleContent } = targetOption;
    if (!(entry in moduleNameCache.current)) {
      ...
      if (!(fullPath in moduleContentCache.current)) {
        ...
        if (fullPath in globalModuleContentCache.current) {
          moduleContentCache.current[fullPath] = globalModuleContentCache.current[fullPath];
          content = moduleContentCache.current[fullPath].originalContent;
        } else {
          const fileContent = await getModuleContent(fullPath);
          content = fileContent;
          moduleContentCache.current[fullPath] = generateModuleContent(fileContent);
          globalModuleContentCache.current[fullPath] = moduleContentCache.current[fullPath];
        }
        return Promise.all(getModuleDeps(content, fullPath).map(({ entry, fullPath }) => moduleLoader({ entry, fullPath }, targetOption)));
      } else {
