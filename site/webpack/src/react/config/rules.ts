...
export const rulesConfig = (restProps: SafeGenerateActionPropsWithReact): RuleSetRule[] => {
  return [cssRules(restProps), jsRules(restProps), cssModuleRules(restProps), resourceRules(restProps)].filter(Boolean) as RuleSetRule[];
};
