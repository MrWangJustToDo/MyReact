...
export const createStore = (
  ...
  const rootStore = createWithEqualityFn<RootState>((set, get) => {
    ...
    const setPerformanceCurrent = (current: number) => set((state) => ({ performance: { ...state.performance, current } }));
    ...
    const rootState: RootState = {
      ...
      setEvents: (events: Partial<EventManager<any>>) => set((state) => ({ ...state, events: { ...state.events, ...events } })),
